...
[INFO] o.a.t.g.s.GremlinServer - x.y.z
         \,,,/
         (o o)
-----oOOo-(4)-oOOo-----
...
[INFO] o.a.t.g.s.t.TransactionManager - TransactionManager initialized with idleTransactionTimeoutMillis=60000, maxTransactionLifetimeMillis=600000, maxTransactions=1000
...
The final startup line reports the server's thread pool configuration.  The worker and boss thread pools default to a
single thread each, while the gremlin pool size shown is not a fixed value.  When `gremlinPool` is not set in the
configuration, Gremlin Server sizes this pool to the number of processors available to the JVM, so the value reported
varies from one machine to another unless `gremlinPool` is assigned an explicit value in the YAML configuration.
