...
image:language-variants.png[width=300,float=right] The `GraphTraversalSource`, referred to as "g" throughout this
documentation, is basically the connection to a graph instance. That graph instance might be
<<connecting-embedded,embedded>>, hosted in <<connecting-gremlin-server,Gremlin Server>> or hosted in a
<<connecting-rgp,RGP>>, but the `GraphTraversalSource` is agnostic to that. The examples that follow are
self-contained and use an <<connecting-embedded,embedded>> `TinkerGraph`, for which a `GraphTraversalSource` can be
created directly in the Gremlin Console:
...
With "g" established, getting data into the graph regardless of programming language or mode of operation is just some
basic Gremlin:
...
Retrieving this data is also just writing a Gremlin statement. The queries below run against the same graph that the
additions above populated, so the "marko" and "stephen" vertices and the "knows" edge between them are already present:
