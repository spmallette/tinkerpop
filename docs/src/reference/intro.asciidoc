...
The embedded reference implementation is distributed through Maven Central under the `org.apache.tinkerpop` group. A
project that embeds a graph depends on `gremlin-core`, which provides the traversal API, together with a graph
implementation such as `tinkergraph-gremlin`. The Maven coordinates are:
...
The equivalent Gradle coordinates are:

[source,groovy]
----
implementation 'org.apache.tinkerpop:gremlin-core:x.y.z'
implementation 'org.apache.tinkerpop:tinkergraph-gremlin:x.y.z'
----

With those dependencies in place, a `Graph` instance is created. The imports required by the code throughout this
section are listed alongside the first statement:
...
// tinkergraph-gremlin module
import org.apache.tinkerpop.gremlin.tinkergraph.structure.TinkerGraph;

Graph graph = TinkerGraph.open();
----

A newly opened `TinkerGraph` is empty, so a traversal such as `g.V()` returns no results until the graph is populated.
Vertices and edges can be added directly through the structure API, and their identifiers, labels, and property values
can be read back through the same interfaces:
...
Object markoId = marko.id();
String markoLabel = marko.label();
String markoName = marko.value("name");
----

Alternatively, one of the sample graphs bundled with TinkerGraph can be loaded with `TinkerFactory` rather than
building the graph by hand. The "modern" toy graph used throughout this documentation is loaded as follows:
