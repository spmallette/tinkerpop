...
The driver is distributed as the `Gremlin.Net` package on NuGet and is installed as shown in <<gremlin-dotnet,Gremlin.Net>>.
The `GremlinClient` and `GremlinServer` types belong to the `Gremlin.Net.Driver` namespace and the
`GraphBinary4MessageSerializer` belongs to the `Gremlin.Net.Structure.IO.GraphBinary4` namespace, so the following
`using` directives are required for the example below to compile:
...
The response serializer is passed to the `GremlinClient` constructor. After the client is created, a traversal can be
submitted and its result retrieved:
