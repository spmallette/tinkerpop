...
The `Cluster` holds the driver configuration and can be built from an Apache Commons `Configuration` object or
from a YAML file. The key names in the table below are the names read from a `Configuration` object, which is
also how the entries of a Java properties file are interpreted. Such a `Configuration` is passed to
...
connectionPool.connectTimeoutMillis=5000
connectionPool.maxConnections=256
auth.type=basic
auth.username=stephen
auth.password=password
----

[source,java]
----
Configuration conf = new Configurations().properties("conf/driver.properties");
Cluster cluster = Cluster.open(conf);
Client client = cluster.connect();
GraphTraversalSource g = traversal().with(DriverRemoteConnection.using(client, "g"));
----

The same settings can be expressed in a YAML file, which is supplied to `Cluster.open(String)` as the path to
that file:
...
A YAML configuration is deserialized field by field rather than by the dotted key names, so a few of its keys
differ from those in the table. The connection pool timeout fields are named `connectTimeout`, `idleTimeout`,
`keepAliveTime` and `readTimeout`, without the `Millis` suffix that the `Configuration` names carry:
...
IMPORTANT: A `Cluster` built from a `Configuration` object requires at least one entry for `hosts` and fails to
build without it. The `localhost` default listed below applies only when the `Cluster` is built programmatically,
for example with `Cluster.open()`.

NOTE: Not every setting is read from a `Configuration` object. The `path`, top-level `username` and `password`,
`interceptors`, `auth.region` and `auth.serviceName` keys are ignored on that route, and the corresponding
values must be supplied through the equivalent `Cluster.Builder` methods such as `path()`, `interceptors()` and
`auth()`. Basic authentication credentials remain available on this route through the `auth.username` and
`auth.password` keys.
...
|connectionPool.keyStoreType |The format of the `keyStore`. Can be: `JKS` or `PKCS12`. |_none_
