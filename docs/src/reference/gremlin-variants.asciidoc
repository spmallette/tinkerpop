...
<<connecting-embedded,embedded>> usage, lambdas can be leveraged directly. The examples in this section assume an
embedded `GraphTraversalSource` together with the imports for the types they reference:
...
g.V().as("a").out("knows").as("b").select("b").by((Function<Vertex, Object>) v -> v.<String>value("name").length()) <3>
----
...
g.V().as("a").out("knows").as("b").select("b").by(Lambda.<Vertex,Object>function("it.value('name').length()"))
----

When the same traversal is submitted repeatedly with only its argument values changing, `GValue` supplies those values
as named parameters in place of literals. Passing a `GValue` to one of the
<<traversal-parameterization,steps that accept parameters>> allows a graph system that supports query caching to reuse a
compiled form of the traversal across submissions. See <<gremlin-java-gvalue,GValue Parameterization>> for the Java form
and the list of parameterizable steps.
