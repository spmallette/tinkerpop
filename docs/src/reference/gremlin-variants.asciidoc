...
languages, GraphBinary is the default and the intended long-term option for response serialization. GraphSON is
also currently selectable, as described in the note below.

IMPORTANT: The Java driver currently retains support for GraphSON, which is useful primarily for testing. GraphBinary
is the intended long-term serialization option for JVM-based languages, and GraphSON support is expected to be removed
once 4.0 is fully released.
...
// A graph provider that exposes custom types supplies an IoRegistry to register them. Substitute the
// provider's registry instance here. When no provider registry is required, the serializer can be built
// with its no-argument constructor: new GraphBinaryMessageSerializerV4().
