...
const { P: p, TextP: textp, column, direction, pick, pop, order, scope, t, cardinality } = gremlin.process;
----
...
The Java pattern of pairing a cardinality with a value, such as `Cardinality.list(value)`, has no equivalent in the
public Gremlin-JavaScript API, so a value cannot be tagged with a cardinality from client code in this variant.
...
This behavior needs to be considered when using Gremlin to return such results. A related consideration arises with
`group()` and `groupCount()`, as in:
...
where "age" is not a property of every vertex. The `by('age')` modulation projects each vertex through its `age`
value, and a vertex that lacks the property produces no value, so it is silently omitted from the result rather than
counted under a `null` key. When that filtering is the intent, it reads more clearly when stated explicitly by
restricting the traversal to the relevant vertices first:
...
Both forms count only the vertices that carry an `age` property, expressing the intent directly rather than relying on
the implicit filtering of the `by()`-modulation.
