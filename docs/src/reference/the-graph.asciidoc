...
////
[[graph]]
[llms-summary="The structure API: graphs, vertices, edges, properties, meta- and multi-properties, transactions, variables, and the features a graph implementation can support."]
= The Graph
...
Each variance is discoverable at runtime by querying the graph's <<features,`Feature`>> set. The example below
checks all four against an in-memory `TinkerGraph`. Any other provider's `Graph` instance reports its own support
in the same way. Three of these accessors return a `boolean`, while multi-label support is reported as a
`LabelCardinality` enum rather than a `supportsXxx` flag:

[gremlin-groovy]
----
graph = TinkerGraph.open()
graph.features().vertex().supportsMultiProperties()    <1>
graph.features().vertex().supportsMetaProperties()     <2>
graph.features().vertex().getLabelCardinality()        <3>
graph.features().vertex().supportsNullPropertyValues() <4>
----

<1> Whether a vertex property key can hold multiple values (multi-properties).
<2> Whether a vertex property can itself carry properties (meta-properties).
<3> The vertex label cardinality (multi-label), returned as a `LabelCardinality` enum rather than a boolean.
<4> Whether an explicit `null` can be stored as a property value (nullable properties).

Analogous accessors exist on `graph.features().edge()` for the variances that also apply to edges. Each variance
subsection below names the specific accessor to use.
...
"name" properties. This is what enables the multi-properties and meta-properties variances introduced above. A
provider reports support for these at runtime through `graph.features().vertex().supportsMultiProperties()` and
`graph.features().vertex().supportsMetaProperties()` respectively.
...
Its full schema and example traversals are documented in the
link:https://tinkerpop.apache.org/docs/x.y.z/data/#the-crew[Crew Graph] section of the TinkerPop Sample Data reference.
...
The configured cardinality can be discovered at runtime via `graph.features().vertex().getLabelCardinality()`,
which returns one of the `LabelCardinality` enum values above rather than a `supportsXxx` boolean.
