...
The printed output groups the supported capabilities by the element they describe: graph features (which
include a nested set of variable features), vertex features (with their vertex property features), and edge
features (with their edge property features). Each group lists the individual capabilities of that element
type as boolean flags, where a value of `true` indicates that the `Graph` supports the associated operation.

A common pattern for using features is to check their support prior to performing an operation. The example
below creates a traversal source `g` from the `graph` and guards a transaction commit behind the relevant
feature check, so that the commit is only attempted when the `Graph` actually supports transactions:
...
The boundary between the flat and hidden namespaces is enforced at write time. A property keyed in the flat
namespace can be written by an end user and read back through the standard traversal API:

[gremlin-groovy,modern]
----
g.V(1).property('location','santa fe')
g.V(1).values('location')
----

A write to a "~"-prefixed key in the hidden namespace is rejected with an `IllegalArgumentException` rather
than being silently accepted or ignored. A hidden key is also filtered out on read, so it never surfaces
through the standard traversal API even when a graph system has populated it. Only a leading "~" marks a key
as hidden, so a "~" appearing elsewhere in a key is treated as an ordinary character.

[source,text]
----
gremlin> g.V(1).property('~metadata','secret')
Property key can not be a hidden key: ~metadata
gremlin> g.V(1).values('~metadata')
gremlin>
----
