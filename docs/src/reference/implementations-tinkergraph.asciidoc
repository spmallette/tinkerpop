...
[source,groovy]
----
graph = TinkerFactory.createModern()      // <1>
g = traversal().with(graph)
graph.createIndex("name", Vertex.class)   // <2>
...
<1> Instantiate the modern toy graph as the `TinkerGraph` instance `graph` and spawn its traversal source `g`. Any
loaded `TinkerGraph` works here. The modern graph is used because the queries below reference its `person` vertices
and `knows` edges.
<2> Create the index once at graph setup time; all subsequent `match()` calls referencing `{name: $who}` use it
