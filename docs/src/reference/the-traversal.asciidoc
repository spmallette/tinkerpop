...
location. The location passed to `io()` is a file path whose extension determines the serialization format, where
`.kryo` selects <<gryo,Gryo>>, `.json` selects <<graphson,GraphSON>>, and `.xml` selects <<graphml,GraphML>>. The
`read()` modulator is inert on its own, so no data is loaded until a terminal step such as `iterate()` triggers
...
The <<io-step,`io()`-step>> documentation provides more complete details on usage, including how the `with()`
modulator overrides the format detection described above.
...
location. The location passed to `io()` is a file path whose extension determines the serialization format, where
`.kryo` selects <<gryo,Gryo>>, `.json` selects <<graphson,GraphSON>>, and `.xml` selects <<graphml,GraphML>>. The
`write()` modulator is inert on its own, so nothing is persisted until a terminal step such as `iterate()` triggers
execution of the traversal.

[gremlin-groovy,modern]
----
g.io('/tmp/tinkerpop-modern.json').write().iterate()
----
