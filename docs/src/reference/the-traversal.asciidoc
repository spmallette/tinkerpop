...
A `GValue` pairs a parameter name with a value and is constructed with one of the static factory methods on the
`GValue` class. `GValue.of(name, value)` detects the type automatically, while typed variants such as
`GValue.ofString(name, value)` and `GValue.ofLong(name, value)` make the type explicit. When a traversal is constructed
in Java, a `GValue` is supplied in place of a literal argument for any of the parameterizable steps listed below.

[source,java]
----
g.V(GValue.of("vid", 1));                                            <1>
g.V().has(GValue.ofString("lbl", "person"), "name", "marko");        <2>
g.V().hasLabel(GValue.ofString("lbl", "person"));                    <3>
g.V().hasLabel("person").limit(GValue.ofLong("count", 2L));          <4>
----

<1> The identifier supplied to `V()` is parameterizable.
<2> The `has(label, key, value)` form parameterizes the element label. The property key and the comparison value are not parameterizable.
<3> The element label supplied to `hasLabel()` is parameterizable.
<4> The count supplied to `limit()` is parameterizable.
...
When a traversal is submitted as a `gremlin-groovy` script rather than constructed in Java, parameterization is
expressed through script bindings instead of `GValue`. The variable names that appear in the script are supplied as a
separate map of bindings. This keeps the script text constant across invocations and prevents the supplied values from
being interpreted as executable Gremlin.

[source,java]
----
Map<String,Object> params = new HashMap<>();
params.put("vid", 1);
client.submit("g.V(vid)", params);
----
...
|<<has-step,has()>> | String label (the element label in the `has(label, key, value)` form)
...
|<<graph-step,V()>> | Object... ids
|===

Only the argument identified for each step is parameterizable. A parameter supplied to an argument that is not
identified above is not rejected. It is instead treated as a literal value, which can silently produce incorrect
results. For example, supplying a parameter as the comparison value of `has(key, value)` compares the property against
the parameter object itself rather than the value it wraps, so the filter matches nothing and the traversal returns an
empty result rather than raising an error.
