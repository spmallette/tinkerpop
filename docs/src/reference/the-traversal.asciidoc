...
Elements written through a partitioned traversal source are automatically tagged with that source's `writePartition`,
which is why the vertex added through "gA" carries a `_partition` value of "a" and the vertex added through "gB"
carries a value of "b". When reading, a traversal source returns only those elements whose `partitionKey` value is
contained in its `readPartitions` set. Elements that do not carry the `partitionKey` property, such as the six
pre-existing vertices of the modern graph, are treated as belonging to no readable partition and are therefore hidden
from a source that restricts its read partitions.
...
`includeMetaProperties` value is set to `true` when the `PartitionStrategy` is built. This behavior is disabled by
default, as `includeMetaProperties` defaults to `false`.  The `partitionKey` will be
