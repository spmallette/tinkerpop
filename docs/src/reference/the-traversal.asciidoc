...
The reducing and collection steps that support scoping are:
...
A number of string steps also accept a `Scope`. For these, `Scope.local` applies the transformation to each
string element within an incoming list and returns a list, while the default behavior operates on a single
incoming string:
...
As scoping is a per-step feature, each step's own section is the authoritative description of how it
interprets `Scope`.
