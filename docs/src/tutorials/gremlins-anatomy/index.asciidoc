...
step and it returns the `Map` constructed in the `groupCount()`-step. Other terminal steps return different results:
`hasNext()` returns a `boolean` reporting whether at least one result remains, `toList()` drains the traversal and
returns all of its results as a `List`, and `iterate()` executes the traversal for its side-effects and returns the
drained `GraphTraversal` itself rather than any results. Without a terminal step, the expression evaluates to a
`GraphTraversal` that describes the computation but has not yet been executed against the graph.

This distinction is easy to overlook when working in the Gremlin Console. When a bare `GraphTraversal` is entered at
the console prompt, the console automatically iterates it and displays the results, so the running example without its
trailing `next()` still appears to produce output. That automatic iteration is a convenience of the console's
interactive prompt rather than a property of the `GraphTraversal` itself. Outside of the console, the `GraphTraversal`
returned by the expression must be iterated explicitly, which is precisely what a terminal step does.
