...
* `maxBarrierSize` - The maximum number of traversers that can be held in the barrier before being processed. It is
expected to be a positive integer (`1` or greater). When the argument is omitted, the barrier is unbounded and holds
the entire traverser stream before releasing it.
...
The `maxBarrierSize` argument is not validated against its expected range. A value of zero or less is accepted without
raising an error, but the barrier then never fills and releases no traversers, so the traversal downstream of the step
produces no results. A positive value is therefore required for the step to emit anything.
