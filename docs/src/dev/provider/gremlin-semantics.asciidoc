...
only `to()` needs to be specified. Unlike `addV()`, a `null` edge label does not raise an error: when the `edgeLabel`
argument is `null`, the default edge label `edge` is used.
...
The example below adds one edge using the mid-traversal form and one using the start-step form:

[gremlin-groovy,modern]
----
g.V(1).addE('knows').to(__.V(2))             <1>
g.addE('created').from(__.V(1)).to(__.V(3))  <2>
----

<1> As a mid-traversal step, the incoming vertex becomes the source of the edge and only `to()` is required.
<2> As a start step, both `from()` and `to()` must be supplied.

*Exceptions:*

* If a label-producing `Traversal` yields `null`, an `Argument Error` is raised.
* If a `from()` or `to()` argument does not resolve to a `Vertex` or to the id of an existing `Vertex` in the graph, a
...
* `has(key, null)` filters to elements where the property value is `null` rather than performing an existence
  check. Such a match is only possible on a graph that permits storing `null` property values. On a graph that does
  not, no property is created for a `null` value and the filter yields no results.
