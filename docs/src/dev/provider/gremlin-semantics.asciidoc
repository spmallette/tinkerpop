...
* `edgeIds` - Zero or more edge identifiers. If none are provided, all edges are returned. When a single `LIST`
  or `SET` is supplied as the sole argument, it is unrolled one level so that each of its elements is treated as
...
* When used mid-traversal with literal identifiers, the content of the incoming traverser is ignored for
  identifier resolution. A fresh global lookup by those identifiers is performed once for each incoming
  traverser, so the same identifiers are resolved again for every traverser that arrives.
* Unrolling of a single collection argument is one level deep. A collection nested inside that argument is not
  unrolled further and is passed through as a single identifier value, which a provider will typically reject as
  invalid.
...
The following example reads edges by literal identifiers, first as separate arguments, then as a single list
that is unrolled one level, and finally with an identifier that matches nothing:
...
* An `Argument Error` is raised when an argument cannot be interpreted as a valid edge identifier by the graph,
  such as a value of an unsupported type or a collection that remains nested after one level of unrolling.
* An `Argument Error` is raised when a traversal argument contains a mutating step, because the child traversal
  that supplies identifiers must be read-only:

[source,groovy]
----
g.E(__.V(1).addE("knows"))
----
