...
provided key. When a single key is provided, the raw sideEffect value referenced by that key is emitted. When multiple
keys are provided, a `MAP<STRING, any>` of sideEffects is emitted, keyed by side-effect name.

Handling of keys that were never registered or populated is asymmetric with respect to the number of keys provided.
When a single key is provided and that key does not exist among the traversal's side-effects, an `Argument Error` is
raised. When multiple keys are provided, keys that do not exist are silently dropped regardless of their position in
the argument list, so the emitted `MAP<STRING, any>` contains only the keys that resolve and may be partial or, if no
keys resolve, empty.

*Exceptions:*

* If a single side-effect key is provided and that key was never registered or populated, an `Argument Error` is
raised.
