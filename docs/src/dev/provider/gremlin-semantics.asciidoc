...
description if run with `verbose=true`. This directory reports only the services a provider has registered, so a graph
with no registered services returns an empty result from `call()` and `call("--list")`. A named service must be
registered before it can be invoked. Providers using their own implementation of the call operation must provide their
