...
* A child traversal supplied as a predicate value in `where(P)` or `where(startKey, P)` must be read-only. Mutating
  steps in such a traversal are rejected with an `Argument Error`.
* This read-only restriction applies only to predicate-value traversals. The existence form `where(Traversal)` does
  not guard its child traversal, so a mutating step such as `where(__.addV(...))` or `where(out().property(...))`
  executes against the graph and mutates it rather than raising an error.
...
* An `Argument Error` is raised if a predicate-value child traversal in `where(P)` or `where(startKey, P)` contains
  mutating steps. The existence form `where(Traversal)` is not subject to this check and raises no error for a
  mutating child traversal.
