...
one result. If any child traversal produces no result, the traverser is filtered out.

[gremlin-groovy,modern]
----
g.V().and(outE("created"), values("age").is(gt(30))).values("name")
----

The step tests only for the existence of output from each child traversal, never for the content of that output. Any
object a child emits satisfies its branch, including a `BOOLEAN` `false` or a `NULL`. A branch filters the traverser
only when it produces an empty stream. In the first query below the traverser survives even though one branch emits
`false`, because both branches still produce a result. In the second query the traverser is filtered because one of its
branches produces nothing.

[gremlin-groovy,modern]
----
g.V(1).and(constant(false), values("name"))
g.V(1).and(values("name"), out("knows").values("age").is(gt(1000)))
----

Boolean value expressions evaluated within a child traversal follow the <<gremlin-semantics-concepts,Comparability>>
semantics, under which an incomputable comparison, such as one against `NaN` or across types, is defined to be `FALSE`.
Such a comparison therefore produces no result and filters the traverser rather than raising an error. The two queries
below differ only in the value each `age` is compared against. The first retains every person whose age exceeds a
computable bound, while the second retains no one because a comparison against `NaN` is incomputable and so is `FALSE`.

[gremlin-groovy,modern]
----
g.V().hasLabel("person").and(values("name"), values("age").is(gt(30))).values("name")
g.V().hasLabel("person").and(values("name"), values("age").is(gt(Double.NaN))).values("name")
----

`and()` is a connective step. Alongside the argument form shown above, it supports an infix form written with no
arguments, in which the traversal immediately preceding the step and the traversal immediately following it are grouped
into its two child branches when traversal strategies are applied. That grouping extends through the steps that follow
the infix `and()`, so a projection or terminal step written after it is absorbed into the second branch rather than
applied to the surviving traverser. In the illustration below the trailing `values("name")` is grouped into the second
branch and evaluated against the `knows` edges reached there, which yields nothing and filters every traverser. To keep
such a step applied to the output of the filter, the connective can be enclosed in `where(and(...))`, or the `and()` can
be positioned ahead of the projection or terminal step.

[source,groovy]
----
// the trailing values("name") is grouped into the second branch of the infix and(), not applied to the result
g.V().outE("created").and().outE("knows").values("name")

// enclosing the connective in where() keeps the projection applied to the surviving traverser
