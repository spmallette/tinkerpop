...
Because the step performs no type checking on the incoming traverser, a value outside the
`Property` domain is neither passed through nor filtered from the traversal stream. Such input
violates the step's domain contract and causes the traversal to fail with an error.
