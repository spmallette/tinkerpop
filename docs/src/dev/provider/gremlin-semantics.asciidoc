...
* `option(Merge.onCreate, MAP)` | `option(Merge.onCreate, Traversal)` - Sets the `onCreate` argument, either directly
as a `MAP` or dynamically as a `Traversal` that resolves to a `MAP`. It is applied only when `searchCreate` finds no
match and a new `Vertex` is created.
* `option(Merge.onMatch, MAP)` | `option(Merge.onMatch, Traversal)` - Sets the `onMatch` argument, either directly as
a `MAP` or dynamically as a `Traversal` that resolves to a `MAP`. It is applied only when `searchCreate` matches an
existing `Vertex`.
...
* The `Start Step` designation in the summary table applies to the `mergeV(searchCreate: MAP)` and
`mergeV(searchCreate: Traversal)` overloads. The zero-arg `mergeV()` overload can only be used mid-traversal, as it
draws its `searchCreate` argument from the incoming `Traverser`.
* An empty `MAP` is the reproducible way to request the match-all and create-with-defaults behavior for `searchCreate`.
Supplying `null` is treated as equivalent to an empty `MAP`, so the empty `MAP` form is preferred where the overloaded
signatures make a bare `null` ambiguous.
* As is common to Gremlin, it is expected that `Traversal` arguments may utilize `sideEffect()` steps.
