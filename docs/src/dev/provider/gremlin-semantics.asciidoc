...
[llms-summary="The formal semantics of the asDate() step: parse the value of the incoming traverser as a DATETIME."]
[[asDate-step]]
=== asDate()

*Description:* Parse the value of the incoming traverser as a datetime, returning a `DATETIME`. ISO-8601 strings and Unix time numbers are supported.
...
A `STRING` is parsed as an ISO-8601 datetime. A `NUMBER` is interpreted as Unix time, that is, the number of milliseconds elapsed since the epoch (1970-01-01T00:00:00Z). A value that is already a `DATETIME` is returned unchanged.

*Exceptions:*

* An `Argument Error` is raised if the incoming traverser is `null`, is a value whose type is not `STRING`, `NUMBER`, or `DATETIME`, is a string that cannot be parsed as an ISO-8601 datetime, or is a number too large to represent as Unix time.
