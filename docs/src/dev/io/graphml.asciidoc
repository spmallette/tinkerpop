...
The following example is a representation of the
link:https://tinkerpop.apache.org/docs/x.y.z/data/#modern[Modern toy graph] in GraphML:
...
A `Graph` is written to GraphML through the `io()`-step. The `io()`-step configures the operation and the
follow-on `write()` step performs the serialization, while the terminating `iterate()` executes the traversal.
The `IO.writer` option selects the GraphML format explicitly, though the `io()`-step can also infer it from a
`.xml` file name extension:

[source,java]
----
g.io("tinkerpop-modern.xml").
    with(IO.writer, IO.graphml).
  write().iterate()
----

Further detail on the `io()`-step and its options is available in the
link:https://tinkerpop.apache.org/docs/x.y.z/reference/#io-step[Reference Documentation].
...
A GraphML file is read into a `Graph` in the same way, with the follow-on `read()` step in place of `write()` and
the `IO.reader` option selecting the GraphML format:
