...
The following example demonstrates a complete round trip on the JVM. It builds a `GryoMapper` for a specific
`GryoVersion` with registration disabled, writes the
link:https://tinkerpop.apache.org/docs/x.y.z/data/#modern[Modern toy graph] to a file with a `GryoWriter`, then reads
the file back into an empty graph with a `GryoReader`:
...
final GryoMapper mapper = GryoMapper.build().version(GryoVersion.V1_0).registrationRequired(false).create();

final Graph graph = TinkerFactory.createModern();
final GryoWriter writer = GryoWriter.build().mapper(mapper).create();
try (final FileOutputStream os = new FileOutputStream("tinkerpop-modern.kryo")) {
    writer.writeGraph(os, graph);
}

final Graph readGraph = TinkerGraph.open();
final GryoReader reader = GryoReader.build().mapper(mapper).create();
try (final FileInputStream is = new FileInputStream("tinkerpop-modern.kryo")) {
    reader.readGraph(is, readGraph);
}
----

The same `GryoMapper` is supplied to both the `GryoWriter` and the `GryoReader` so that their registrations align. The
`graph.io(IoCore.gryo())` builder offers an equivalent, more concise path to the same readers and writers when a
`Graph` instance is already available.
...
to the JVM. The link:https://github.com/apache/tinkerpop/blob/master/gremlin-core/src/main/java/org/apache/tinkerpop/gremlin/structure/io/gryo/GryoVersion.java[GryoVersion]
