...
The examples in this section run against an in-memory TinkerGraph. The following creates an empty graph and the
traversal source `g` used throughout the section:
...
<4> Find all the events from May 31, 2016 through June 1, 2016, with both boundary days inclusive.
