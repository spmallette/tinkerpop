...
----
g.V().                                                                      <1>
    map(__.emit().repeat(__.both().simplePath()).dedup().order().by(id).fold()).    <2>
    dedup()                                                                 <3>
----

<1> The `g.V()` scan visits every vertex in the graph and treats each one as the starting point for the discovery
of a component.

<2> For a given starting vertex the `emit()`-step outputs that vertex and the `repeat(both().simplePath())` clause
walks outward across edges in both directions until no further vertex can be reached without revisiting one. The result
is every vertex that belongs to the same weakly connected component as the starting vertex, including the starting
vertex itself and any isolated vertex that has no neighbors. Ordering those vertices by their identifier with
`order().by(id)` and collecting them with `fold()` produces a canonical list, so that every vertex of a component
yields an identical list.

<3> Because every vertex of a component maps to the same canonical list, the final `dedup()` reduces those duplicates
to a single entry per component and returns one list of vertices for each weakly connected component.
