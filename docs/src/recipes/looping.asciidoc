...
under the License.
////
[llms-summary="Recipe: One common use case when working with Gremlin is to perform complex looping statements using the repeat() step."]
[[looping]]
== Looping
...
satisfies the `hasId('C')` condition since it's reached vertex C.  If that exit condition is instead changed 
to vertex G, the traversal continues through vertex C on to vertex E, following the path A->B->C->E.  Because 
vertex G is unreachable from vertex A by way of `out()` edges, the traversal exits upon reaching the maximum 
number of loops (3) and returns `v[E]`.
...
be accomplished.  They continue to run against the tree graph constructed at the start of the <<looping>>
section, whose `hasParent` edges form the hierarchy `A->B->C->E->F` with `D->C` and `G->F`.  The result counts
below follow directly from that shape, so keeping the topology in mind makes them predictable.
...
The `both().times(2)` loop reaches vertices `A` and `C`, since a two-hop walk over the undirected `hasParent`
edges from `A` returns to `A` and also arrives at `C`.  Each of those vertices then follows its `out()` edges
toward the root, and both paths terminate at vertex `F`.  The traversal therefore returns `v[F]` once for each
of the two vertices the first loop produced.
