...
2. The Spark on YARN runtime dependencies bundled with the Spark plugin still need to be delivered to the YARN
containers, which `spark-submit` would normally handle.
3. Resolving reason 2 by adding the cluster's own Spark jars to the classpath may create all kinds of version
...
This recipe uses the Gremlin Console with the
link:https://tinkerpop.apache.org/docs/x.y.z/reference/#spark-plugin[Spark plugin] (the
link:https://tinkerpop.apache.org/docs/x.y.z/reference/#hadoop-plugin[Hadoop plugin] is optional). A fresh binary
distribution does not ship with the Spark plugin installed, so install it from within the Gremlin Console:
...
Then restart the console and activate the plugin:
...
Installing the plugin creates the `ext/spark-gremlin/lib` folder that the archive step in the "Running the job"
section below relies on. If your Hadoop cluster has been configured to use file compression, e.g. LZO compression,
copy the relevant jar (e.g. `hadoop-lzo-*.jar`) into that same `ext/spark-gremlin/lib` folder.
...
export HADOOP_HOME=/usr/local/lib/hadoop-3.4.3
export HADOOP_CONF_DIR=/usr/local/lib/hadoop-3.4.3/etc/hadoop
...
==>/usr/local/lib/hadoop-3.4.3
gremlin> hadoopConfDir = System.getenv('HADOOP_CONF_DIR')
==>/usr/local/lib/hadoop-3.4.3/etc/hadoop
...
files (e.g. `spark-core_2.13-4.1.2-some-vendor.jar` instead of `spark-core_2.13-4.1.2.jar`). Of course, TinkerPop will
