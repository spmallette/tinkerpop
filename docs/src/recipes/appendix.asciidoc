...
The second traversal supplied to `union()` handles those neighbors that have no `occured_at` edge. For each such
neighbor, `coalesce()` first attempts to traverse to an adjacent vertex labelled "dimon" and returns that child vertex
when one is present. This is why neighbor "C", which has no datetime but does have a "dimon" child, is represented in
the results by that child "F" rather than by "C" itself. When a neighbor has no "dimon" child, `coalesce()` falls back
to `identity()` and returns the neighbor unchanged, as is the case for "B".
